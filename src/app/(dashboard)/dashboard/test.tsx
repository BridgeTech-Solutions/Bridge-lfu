    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
      ...
      <div className="mb-8">
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 ${stablePermissions.canViewAllData ? 'lg:grid-cols-4' : 'lg:grid-cols-3'}`}>
            ...
            loading ? (
              <StatCardSkeleton />
                  ...
                  <Card>
                    <CardContent className="p-6">
                      <p className="text-sm text-gray-600">{statsTranslations.t('totalClients')}</p>
                      <p className="text-3xl font-bold mt-1">{(stats?.total_clients || 0).toLocaleString('fr-FR')}</p>
                    </CardContent>
                  </Card>
            )
              ) : null}
          {licenseLoading ? (
            <StatCardSkeleton />
                ...
                <Card>
                  <CardContent className="p-6">
                    <p className="text-sm text-gray-600">{statsTranslations.t('activeLicenses')}</p>
                    <p className="text-3xl font-bold mt-1">{operationalLicenseCount.toLocaleString('fr-FR')}</p>
                    <p className="text-xs text-gray-500 mt-1">
                      {statsTranslations.t('activeLicensesSubtitle').replace('{{count}}', String(licenseStats?.by_status.expired || 0))}
                    </p>
                  </CardContent>
                </Card>
          )}
          {equipmentLoading ? (
            <StatCardSkeleton />
                ...
                <Card>
                  <CardContent className="p-6">
                    <p className="text-sm text-gray-600">{statsTranslations.t('activeEquipment')}</p>
                    <p className="text-3xl font-bold mt-1">{operationalEquipmentCount.toLocaleString('fr-FR')}</p>
                    <p className="text-xs text-gray-500 mt-1">
                      {statsTranslations.t('activeEquipmentSubtitle').replace('{{count}}', String(equipmentStats?.by_status.obsolete || 0))}
                    </p>
                  </CardContent>
                </Card>
          )}
          {licenseLoading || equipmentLoading ? (
            <StatCardSkeleton />
                ...
                <Card>
                  <CardContent className="p-6">
                    <p className="text-sm text-gray-600">{statsTranslations.t('criticalAlerts')}</p>
                    <p className="text-3xl font-bold mt-1">{statsAlerts.criticalCount.toLocaleString('fr-FR')}</p>
                    <p className="text-xs text-gray-500 mt-1">
                      {statsTranslations.t('criticalAlertsSubtitle').replace('{{count}}', String(statsAlerts.warningCount))}
                    </p>
                  </CardContent>
                </Card>
          )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <ChartCard title={chartsTranslations.t('equipmentByType')}>
              <div className="h-[300px]">
                <TremorDonutChart
                  ...
                  onValueChange={(v) => {
                    // v can be string (index) or object depending on Tremor version; handle both
            ...
            <ChartCard title={chartsTranslations.t('licenseStatus')}>
              <div className="h-[300px]">
                <TremorBarChart
                  className="h-full"
                  data={[
                    { name: 'Actif', value: (licenseStats.chart_data.statuses.find(s => s.name === 'active')?.value || 0) + (licenseStats.by_status.about_to_expire || 0) },
          ...
          {/* Nouveau graphique - Évolution des expirations (12 mois via licenseTrends) */}
            ...
            <ChartCard title={chartsTranslations.t('equipmentStatus')}>
              <div className="h-[300px]">
                <TremorBarChart
                  className="h-full"
                  data={[
                    { name: 'Actif', value: (equipmentStats.chart_data.statuses.find(s => s.name === 'actif')?.value || 0) + (equipmentStats.by_status.bientot_obsolete || 0) },
              ...
              {licenseTrends.loading ? (
                ...
                <ChartCard title={chartsTranslations.t('upcomingExpirations')}>
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={licenseTrends.months}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="month" />
                      <YAxis />
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="count" stroke="#FF8042" strokeWidth={2} name={chartsTranslations.t('expirationsSeries')} />
                    </LineChart>
                  </ResponsiveContainer>
                </ChartCard>
              )}
          ...
          <TabsContent value="equipment" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {equipmentTrends.loading ? (
                ...
                <ChartCard title={chartsTranslations.t('equipmentObsolescence')}>
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={equipmentTrends.months}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="month" />
                      <YAxis />
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="count" stroke="#2563EB" strokeWidth={2} name={chartsTranslations.t('equipmentObsolescenceSeries')} />
                    </LineChart>
                  </ResponsiveContainer>
                </ChartCard>
              )}
              {equipmentLoading ? (
                ...
                <ChartCard title={chartsTranslations.t('equipmentByType')}>
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie data={equipmentStats?.chart_data?.types || []} cx="50%" cy="50%" labelLine={false} label={({ name, percentage }) => `${name} (${percentage}%)`} outerRadius={80} fill="#8884d8" dataKey="value">
